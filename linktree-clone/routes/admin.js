...
router.get('/dashboard', isAuthenticated, async (req, res) => {
  try {
    ...
    res.render('admin/dashboard', { links });
  } catch (error) {
    console.error(error);
...
router.post('/links', isAuthenticated, async (req, res) => {
  try {
    ...
    await Link.create({ title, url, icon, order });
    res.redirect('/admin/dashboard');
  } catch (error) {
    console.error(error);
    res.status(500).send('Server error');
...
// Edit link page
router.get('/links/:id/edit', isAuthenticated, async (req, res) => {
  try {
    const link = await Link.findByPk(req.params.id);
    if (!link) {
      return res.status(404).send('Link not found');
    }
    res.render('admin/edit-link', { link });
  } catch (error) {
    console.error(error);
    res.status(500).send('Server error');
  }
...
router.post('/links/:id', isAuthenticated, async (req, res) => {
  try {
    ...
    await link.update({ title, url, icon, order });
    res.redirect('/admin/dashboard');
  } catch (error) {
    console.error(error);
    res.status(500).send('Server error');
